(function(scope) {
	...
	scope.SelectUIBehavior = {
		...
		/**
		 * Attaches an on scroll handler on window (for recalculating dropdown's position).
		 */
		attached: function() {
			this.listen(window, 'scroll', '_updateDropdownPosition');
		},
		
		/**
		 * Cleans up the element (detaches custom event handlers).
		 */
		detached: function() {
			this.unlisten(window, 'scroll', '_updateDropdownPosition');
		},
		...
		_updateDropdownPosition: function() {
			...
			box.style.top = (thisBounds.top + thisBounds.height + 5) + "px";
			
			// compute maximum width / height of the box to fit inside the window
			var maxWidth = window.innerWidth - box.offsetLeft - 1;
			var maxHeight = window.innerHeight - box.offsetTop - 1;
