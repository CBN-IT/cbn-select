(function(scope) {
	...
	scope.SelectUIBehavior = {
		...
		_openDropdown: function() {
			...
			this.debounce('_openDropdown', this._updateDropdownPosition);
		},
		
		/**
		 * Computes the position of the dropdown box.
		 */
		_updateDropdownPosition: function() {
			...
			box.style.left = (thisBounds.left) + "px";
			box.style.top = (thisBounds.top + thisBounds.height) + "px";
			
			// compute maximum width / height of the box to fit inside the window
			var boxBounds = box.getBoundingClientRect();
			var maxWidth = window.innerWidth - boxBounds.left - 5;
			var maxHeight = window.innerHeight - boxBounds.top - 5;
			
			// set the new dimensions
			box.style.maxWidth = maxWidth;
			this._setDropdownHeight(maxHeight);
			
			// final touch: if the box leaves the viewport, show it on top of the select input
			if ((box.offsetTop + box.clientHeight) > window.innerHeight) {
				box.style.height = '';
				box.style.maxHeight = '';
				this._setDropdownHeight(thisBounds.top - 5);
				box.style.top = (thisBounds.top - box.clientHeight - 1) + 'px';
			}
		},
		
		/**
		 * Changes the dropdown box's height.
		 * 
		...
		_setDropdownHeight: function(newHeight) {
			...
			if (this.useIronList) {
				...
				if (newHeight > this.$.ironList.$.items.clientHeight) {
					// make sure to add the 5px padding of .item:last-of-type
					box.style.height = (this.$.ironList.$.items.clientHeight + 5) + 'px';
					this.$.ironList.fire('resize');
				}
			} else {
