(function(scope) {
	...
	scope.SelectUIBehavior = {
		...
		observers: [
			'_ui_selectedOptionsChanged(_selectedOptions.splices)',
		 ...
		 */
		_ui_selectedOptionsChanged: function(changeRecord) {
			// update it with metadata defaults
			if (changeRecord) changeRecord.indexSplices.forEach(function(s) {
				for (var i= s.index; i < s.index + s.addedCount; i++) {
					this.set('_selectedOptions.'+i+'.highlighted', !!this._selectedOptions[i].highlighted);
					this.set('_selectedOptions.'+i+'.editing', !!this._selectedOptions[i].editing);
				}
			}.bind(this));
