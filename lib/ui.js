(function(scope) {
	...
	/**
	 * An internal behavior that implements the `cbn-select`'s user interface controller.
	 */
	scope.SelectUIBehavior = {
		...
		_ui_selectedOptionsChanged: function() {
			if (this._currentTransactionIndirect && this._currentTransactionIndirect.selectionChanged === false) {
				return;
			}
		...
		_setDropdownWidth: function(availableXSpace) {
			/** @namespace this.$.optionsWrapper */
			var box = this.$.optionsWrapper;
			var selectWidth = this.offsetWidth, reqWidth;
			...
			if (this.useIronList) {
				...
				reqWidth = this.$.ironList.$.items.scrollWidth + scrollSize * 2 + 1;
				// reset the props
				this.$.ironList.style.whiteSpace = '';
				this.$.ironList.$.items.style.overflow = '';
				
				// check if it fits
				if (reqWidth < selectWidth)
					reqWidth = selectWidth;
				...
				reqWidth = (box.offsetWidth > selectWidth ? box.offsetWidth : selectWidth)
				box.style.width = reqWidth + 'px';
				box.style.overflow = '';
				box.style.minWidth = '';
			}
		 ...
		 */
		_removeSelectedOption: function(selectedIdx) {
			...
			if (selectedIdx >= this._displaySelectedOptions.length)
				return;
			this.deselect(this._displaySelectedOptions[selectedIdx].item);
