				...
				callbacks: {
					updateDimensions: function(boundingBox, refBoundingBox, metric) {
						if (metric.dir === undefined || metric.dir == 0) {
							// set max width
							box.style.maxWidth = Math.round(metric.availableSpace[0] - 1) + 'px';
							boundingBox.width = box.offsetWidth;
						}
						if (metric.dir === undefined || metric.dir == 1) {
							...
							console.trace('update height', newHeight, boundingBox.height, metric.collision);
						}
					}.bind(this)
				}
			});
