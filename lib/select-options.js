(function(scope) {
	...
	/**
	 * An internal behavior that implements the `cbn-select`'s options / values-related functionality.
	 */
	scope.SelectOptionsBehavior = {
		...
		/**
		 * Used to prevent calling the `_reselectCurrentValue` routine.
		 */
		_skipReSelection: false,
		...
		_valueChanged: function(newVal, oldVal) {
			...
			try {
				...
				if (result) {
					 ...
					 * options.
					 */
					if (valueDelayed && this.dataSource && (!this._options || !this._options.length) &&
							(this._dataSource && !this._dataSource._forDelayedValue)) {
						// queue a query request
						this.async(function() {
		...
		_commitValue: function(indirect) {
			...
			// fire the `cbn-selection-changed` event, which allows preventing the operation
			var newValueNorm = newValue;
			if (!this.multiple) {
				var newValueNorm = (newValueNorm.length ? newValueNorm[0] : '');
			}
			var changedEv = this.fire('cbn-before-select', {
					newValue: newValueNorm,
					invalid: false,
					indirect: indirect
				}, {
					bubbles: true,
			...
			if (changedEv.defaultPrevented) {
				// roll back
				this._endTransaction();
				return !changedEv.detail.invalid;
			}
		...
		_dataSourceInitializedCallback: function() {
			var initialData = /**@type {Array}*/(this._dataSource.filteredData);
			if (!initialData) initialData = [];
			this._setOptions(initialData);
			this._filterOptions(/**@type {String}*/this._currentFilterValue);
		...
		_setOptions: function(items) {
			...
			if (this._delayedValue) {
				this._setIndirectValue(this._delayedValue, { indirect: true, delayed: true });
				if (this._dataSource)
					this._dataSource._forDelayedValue = false;
				
			} else if (!this._skipReSelection) {
		...
		_filterOptions: function (query) {
			...
			this._currentFilterValue = query;
			var filteredOptions = this.$.filter.filter(this._options, query);
