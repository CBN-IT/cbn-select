(function(scope) {
	...
	scope.SelectOptionsBehavior = {
		 ...
		 * @type Function
		 * @param {Array} selectedOptions The selected options to be committed.
		 * @param {Array|Boolean} The altered selected options array or false to roll back.
		 */
		...
		_commitValue: function(indirect) {
			if (this._commitValueCallback) {
				var result = this._commitValueCallback(this._currentTransactionItems);
				if (!result) {
					// just roll back
					this._endTransaction();
					return;
				}
				this._currentTransactionItems = result;
				// recalculate the hashes from the altered selected items
