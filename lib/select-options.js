(function(scope) {
	...
	scope.SelectOptionsBehavior = {
		...
		/**
		 * Called when the selected value is changed directly.
		 * 
		...
		_valueChanged: function(newVal, oldVal) {
			...
			if (this.dataSource || this._options === undefined) {
				/*
				 * Only set the delayed value if:
				 * - we have no active options; AND
				 * - the value is not null/empty (otherwise, no need to delay); AND
				 * - the server hasn't responded from a delayed value query.
				 *   (care must be taken here: the `_dataSource` might not have been initialized yet!)
				 */
				if (!this._isNUE(this.value) && (!this._options || !this._options.length) && 
		...
		_optionsChanged: function(newOptions, oldOptions) {
			this._setOptions(this.options, (oldOptions === undefined));
		...
		_optionsSpliced: function(splices) {
			...
			this._setOptions(this.options, false);
			this._filterOptions(/**@type {String}*/this._currentFilterValue);
		},
		 ...
		 * @param {Boolean} admitDelayValue If true and delay value is set, restore it whatever.
		 */
		_setOptions: function(items, admitDelayValue) {
			...
			if (this._delayedValue) {
				...
				if (admitDelayValue || (this._dataSource && this._dataSource._forDelayedValue)) {
